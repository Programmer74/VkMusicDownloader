...
class TagEnhancer(
    ...
    fun tryGetAlbumCover(id: String): ByteArray? {
        ...
        try {
            ...
            var imageName = content.getElementById("page").getElementById("sidebar").getElementsByClass("cover-art").first().child(0).attr("href")
    ...
    fun tryGetAlbumInfo(requestedArtist: String, requestedTitle: String): TrackAlbumInfo? {
        ...
        try {
            ...
            val table = Jsoup.parse(body).body().getElementById("content").getElementsByClass("tbl").first()
